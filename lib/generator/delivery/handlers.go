...
func (h *{{ .EntityName }}Handlers) DefineRoutes(r gin.IRouter) {
	r.Use(client_errors.ErrorHandlingMiddleware())
    r.POST("/", h.Create)
	r.GET("/:id", h.Get) 
    r.PUT("/", h.Update) 
...
func (h *{{ .EntityName }}Handlers) Get(c *gin.Context) { 
    ...
    if err != nil {
        c.Error(client_errors.ErrInvalidIDParam) 
        return 
    }
...
func GenerateHandlers(ent schema.Entity, out io.Writer, entityImport, clientErrorsImport string) {
	templateData := struct {
		EntityName         string
		EntityImport       string
		ClientErrorsImport string
	}{
		EntityName:         ent.Name,
		EntityImport:       entityImport,
		ClientErrorsImport: clientErrorsImport,
	}
